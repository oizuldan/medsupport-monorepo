...
- **Partner form backend.** The `/partner` form (and the Home contact form) validate + confirm client-side but do **not** submit anywhere (`console.info` + `// TODO`). Wire a real endpoint (`apps/server` handler or a Next API route) with validation, anti-spam, and email/DB persistence.
